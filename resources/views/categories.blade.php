...
@section('content')
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        <button id="btnAddCategory" onclick="document.getElementById('createModal').classList.remove('hidden')" class="hidden bg-brand-primary hover:bg-cyan-400 text-black font-semibold px-4 py-2 rounded-xl shadow-[0_0_15px_rgba(0,212,255,0.4)] transition-all hover:-translate-y-0.5 flex items-center">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        if (userRole === 'admin' || userRole === 'magasinier') {
            document.getElementById('btnAddCategory')?.classList.remove('hidden');
        }
